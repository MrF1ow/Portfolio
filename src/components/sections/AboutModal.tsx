...
export default function AboutModal({ open, onClose }: AboutModalProps) {
  ...
  const activeTabContent: AboutTabContent = aboutTabs.find((t) => t.id === activeTab) ?? aboutTabs[0]!;
  ...
  return (
    ...
    >
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="about-modal-heading" className="relative z-10 bg-dark text-dark-text w-full max-w-4xl h-[520px] flex flex-col sm:flex-row animate-in overflow-hidden" style={{ animationDuration: "0.3s" }}>
        {/* Mobile top bar */}
        <div className="sm:hidden flex items-center justify-between shrink-0 px-4 h-14 border-b border-border-dark">
          <button onClick={() => setMobileNav(!mobileNav)} aria-label="Toggle navigation" className="w-8 h-8 flex items-center justify-center text-dark-muted hover:text-dark-text transition-colors cursor-pointer">
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <path d="M3 5h12M3 9h12M3 13h12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </button>
          <button onClick={onClose} aria-label="Close" className="w-8 h-8 flex items-center justify-center text-dark-muted hover:text-dark-text transition-colors cursor-pointer">
        ...
        {/* Desktop close */}
        <button onClick={onClose} aria-label="Close" className="hidden sm:flex absolute top-4 right-4 z-30 w-8 h-8 items-center justify-center text-dark-muted hover:text-dark-text transition-colors cursor-pointer">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M12 4L4 12M4 4l8 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
          </svg>
        </button>
        ...
        <div ref={contentRef} className="flex-1 min-h-0 p-8 sm:p-10 overflow-y-auto scrollbar-hide">
          <div className={`transition-all duration-150 ${contentFade ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1"}`}>
            <h3 id="about-modal-heading" className="font-sans text-2xl font-medium tracking-tight mb-6">{activeTabContent.heading}</h3>
            {activeTabContent.content.map((block, i) => <BlockRenderer key={i} block={block} />)}
          </div>
        </div>
      </div>
    </div>
